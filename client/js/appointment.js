...
function cancelAppointmentConfirmDetails(id){
  deleteAppointmentConfirmDetails(id,'cancel')
}
function confirmAppointmentConfirmDetails(id){
  deleteAppointmentConfirmDetails(id,'confirm')
}
function deleteAppointmentConfirmDetails(id,text) {
  ...
  if (confirm('Are you sure want to '+text+'?')) {
    ...
    if (text == "cancel") {
      appointment_details += '&status=CANCELLED&deleted=0&active=1&cancelled_by='+localStorage.getItem('usertype');
    }else if (text == 'delete') {
      appointment_details += '&status='+ data.status+'&deleted=1&active=0';
    }else if (text == 'confirm') {
      appointment_details += '&status=CONFIRMED&deleted=0&active=1'
    };
...
function renderAppointmentsGrid(){
  ...
  var source = {
    ...
    datafields: [{
      ...
      type:"number"},{name:"description",type:"string"},{name:"edit",type:"number"}],
        cache: false,
      ...
      },  
      {
        filterable: false,
          ...
          cellsRenderer: function (row, column, value, rowData) {
            ...
            var full_name = "<div style='margin: 10px;' class='foradmin'><b>" + rowData.full_name + "</b></div>";
            var Email = "<div style='margin: 10px;' class='foradmin'>" + rowData.email+"(" + rowData.phone_number + ")</div>";
            var appointment = "<div style='margin: 10px;'>" + moment(rowData.appointment_time).format('ddd, MMM D, LT') + "</div>";
            var description = "<div style='margin: 10px;'>" +rowData.description+ "</div>";
            var status = "<div style='margin: 10px;'> " + rowData.status + "</div>";
             var confirmbutton = '<div class="gridicons" style="margin: 15px 10px;"></div>';
            var cancelbutton = '<div class="gridicons" style="margin: 15px 10px;"></div>';
            if (rowData.edit == 1) {
              cancelbutton = '<div class="gridicons" style="margin: 15px 10px;"><a href="javascript:void(0)" onClick="cancelAppointmentConfirmDetails('+row+');" class="btn btn-default btn-sm"><i class="fa fa-times-circle-o"></i> Cancel&nbsp;</a></div>';
              if (localStorage.getItem('usertype') == 'Super Admin') {
                confirmbutton = '<div class="gridicons" style="margin: 15px 10px;"><a href="javascript:void(0)" onClick="confirmAppointmentConfirmDetails('+row+');" class="btn btn-default btn-sm"><i class="fa fa-thumbs-up"></i> Confirm</a></div>';
              }
            }
            leftcolumn +=image;
            leftcolumn += status;
            ...
            lastcolumn += cancelbutton;
            lastcolumn += confirmbutton;
            leftcolumn += "</div>";
            rightcolumn += "</div>";
            lastcolumn += "</div>";
...
var enumerateDaysBetweenDates = function(startDate, endDate,disableday) {
  var now = startDate, dates = [];
  while (now.isSameOrBefore(endDate)) {
    if (now.isoWeekday() == disableday) {
      now.add(1, 'days');
     // endDate.add(1, 'days');
    }
...
function getDateRangesForGivenDates(){
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      var availdates = enumerateDaysBetweenDates(startDate, endDate,disableday);
